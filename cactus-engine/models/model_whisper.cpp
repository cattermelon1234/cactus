...
void WhisperModel::reset_graph_side_cache_nodes() {
    // v2 stores autoregressive decoder state inside graph-owned KV cache nodes.
}
...
size_t WhisperModel::build_decoder_self_attention(CactusGraph* gb, size_t input, uint32_t layer_idx, ComputeBackend backend, bool use_cache, size_t position_offset){
    ...
    size_t attn_out_4d = 0;
    if (use_cache &&
        layer_idx < graph_cache_k_nodes_.size() &&
        graph_cache_k_nodes_[layer_idx] != 0) {
        gb->kv_cache_append(k_4d, graph_cache_k_nodes_[layer_idx], cache_window_size_, cache_sink_size_);
        gb->kv_cache_append(v_4d, graph_cache_v_nodes_[layer_idx], cache_window_size_, cache_sink_size_);
        attn_out_4d = gb->attention_cached(
            q_4d,
            k_4d,
            v_4d,
            graph_cache_k_nodes_[layer_idx],
            graph_cache_v_nodes_[layer_idx],
            attention_scale_,
            position_offset,
            cache_window_size_);
    } else {
...
size_t WhisperModel::run_decoder_step(const std::vector<uint32_t>& tokens, bool use_cache, bool last_token_only) {
    ...
    size_t start_idx = (use_cache && cache_total_seq_len_ > 0 && full_len > 1) ? full_len - 1 : 0;
    ...
    float* /*out_token_time_end*/)
{
    ...
    if (cold_start)
    {
        ...
        first_decode_step_ = true;
        run_encoder(audio_features);
        logits_node = run_decoder_step(full_tokens, true, false);
        encoder_ready_ = true;
    }
    else
    {
    ...
    cache_total_seq_len_ += last_new_tokens_;
