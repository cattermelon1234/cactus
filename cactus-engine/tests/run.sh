...
echo ""
echo "Step 1: Checking model weights..."
MODEL_DIR=$(echo "$MODEL_NAME" | sed 's|.*/||' | tr '[:upper:]' '[:lower:]')
WEIGHTS_DIR="$PROJECT_ROOT/weights/$MODEL_DIR"
if [ -d "$WEIGHTS_DIR" ]; then
    echo "Model weights found at $WEIGHTS_DIR"
else
    ...
    if command -v cactus &> /dev/null; then
        ...
        echo "Install the cactus CLI or download weights manually."
        exit 1
    fi
fi
...
export CACTUS_TEST_ASSETS="$PROJECT_ROOT/cactus-engine/tests/assets"
export CACTUS_INDEX_PATH="$PROJECT_ROOT/cactus-engine/tests/assets"
