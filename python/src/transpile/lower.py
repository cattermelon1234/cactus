...
#Need to work on this for mmmaps and constants that can be bound as tensors vs materialized as literals -- Fuck this shit I fucking hate my life
...
def _lower_ir_node(g: Graph, node: IRNode, env: dict[str, Any], ir: IRGraph) -> list[Any]:
    ...
    if op == "scalar_add":
        return [g.scalar_add(_ensure_fp16_tensor(g, _tensor(env, node.inputs[0])), float(node.attrs["value"]))]

    if op == "scalar_subtract":
        return [g.scalar_subtract(_ensure_fp16_tensor(g, _tensor(env, node.inputs[0])), float(node.attrs["value"]))]
    ...
    if op == "scalar_multiply":
        return [g.scalar_multiply(_ensure_fp16_tensor(g, _tensor(env, node.inputs[0])), float(node.attrs["value"]))]

    if op == "scalar_divide":
        return [g.scalar_divide(_ensure_fp16_tensor(g, _tensor(env, node.inputs[0])), float(node.attrs["value"]))]

    if op == "scalar_floor_divide":
        source = _tensor(env, node.inputs[0])
        x = _ensure_fp16_tensor(g, source)
    ...
    if op == "scalar_not_equal":
        return [g.scalar_not_equal(_ensure_fp16_tensor(g, _tensor(env, node.inputs[0])), float(node.attrs["value"]))]

    if op == "scalar_equal":
        not_equal = g.scalar_not_equal(_ensure_fp16_tensor(g, _tensor(env, node.inputs[0])), float(node.attrs["value"]))
    ...
    if op == "matmul":
        ...
        if legalized is None:
            static_batched = _lower_static_batched_matmul(g, lhs, rhs)
    ...
    if op == "masked_fill":
        ...
        fill_term = g.scalar_multiply(_ensure_tensor_dtype(g, mask, base.dtype), float(node.attrs["value"]))
...
def _try_lower_prefix_mask_advanced_index(
    ...
    if source_shape[: len(mask_shape)] != mask_shape:
        return None
    return g.masked_select_prefix(source, mask)
...
def _try_lower_advanced_index(
    ...
    if lowered is not None:
        return lowered
    return _try_lower_prefix_mask_advanced_index(g, node, env)
...
def _lower_where_branch_term(g: Graph, branch_value: Any, mask: Tensor, *, dtype: int | None) -> Tensor:
    ...
    return g.scalar_multiply(mask, float(branch_value))
...
def _lower_static_batched_matmul(g: Graph, lhs: Tensor, rhs: Tensor) -> Tensor | None:
    ...
    def _build(dim: int, coord_prefix: tuple[int, ...]) -> Tensor:
        if dim == rank:
            ...
            out = _matmul_with_quantized_rhs_legalization(g, lhs_slice, rhs_slice)
