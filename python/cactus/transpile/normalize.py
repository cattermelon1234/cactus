...
def normalize_target(target: str) -> str:
    ...
    if target.startswith("aten.detach"):
        return "identity"
    if target.startswith("aten.dropout"):
        return "identity"
    if target.startswith("aten.native_dropout"):
        return "identity"
    ...
    if target.startswith("aten.logical_and") or target.startswith("aten.__and__"):
        return "logical_and"
    if target.startswith("aten.logical_not") or target.startswith("aten.bitwise_not"):
        return "logical_not"
    if target.startswith("aten.abs"):
        return "abs"
    if target.startswith("aten.clamp"):
        return "clamp"
    if target.startswith("aten.where"):
        return "where"
    if target.startswith("aten.masked_scatter"):
        return "masked_scatter"
    if target.startswith("aten.masked_fill"):
        return "masked_fill"
    ...
    if target.startswith("aten.sqrt"):
        return "scalar_sqrt"
    if target.startswith("aten.reciprocal"):
        return "reciprocal"
    ...
    if target == "aten.flatten.using_ints":
        return "flatten"
    if target.startswith("aten.squeeze"):
        return "squeeze"
    ...
    if target.startswith("aten.permute"):
        return "permute"
    if target.startswith("aten.numpy_T"):
        return "numpy_T"
    if target.startswith("aten.movedim"):
        return "movedim"
    ...
    if target.startswith("aten.pad"):
        return "pad"
    if target.startswith("aten.one_hot"):
        return "one_hot"
    if target.startswith("aten.tril"):
        return "tril"
    if target.startswith("aten.unfold"):
        return "unfold"
    if target.startswith("aten.embedding"):
        return "embedding"
    if target.startswith("aten.conv1d"):
        return "conv1d"
    if target.startswith("aten.conv2d"):
        return "conv2d"
