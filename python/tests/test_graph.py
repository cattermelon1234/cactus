...
class TestGraphTensorOps(unittest.TestCase):
    ...
    def test_not_equal(self):
        ...
        a_data = np.array([[1, 2, 3], [4, 5, 6]], dtype=np.float16)
        b_data = np.array([[1, 0, 3], [0, 5, 7]], dtype=np.float16)
        g.set_input(a, a_data)
        g.set_input(b, b_data)
        g.execute()

        out = y.numpy()
        expected = (a_data != b_data).astype(np.float16)
        np.testing.assert_allclose(out, expected, atol=1e-2)

    def test_scalar_not_equal(self):
        g = Graph()
        a = g.input((2, 3))
        y = g.scalar_not_equal(a, 0.0)

        a_data = np.array([[0, 2, 0], [4, 0, 6]], dtype=np.float16)
        g.set_input(a, a_data)
        g.execute()

        out = y.numpy()
        expected = (a_data != 0.0).astype(np.float16)
        np.testing.assert_allclose(out, expected, atol=1e-2)
