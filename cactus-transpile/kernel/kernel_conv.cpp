...
void fp16_rowmajor_gemm_rhs_transposed_to_fp32(
    const __fp16* lhs,
    const __fp16* rhs,
    float* output,
    size_t M,
    ...
    std::vector<float> rhs_f32(N * K);
    for (size_t i = 0; i < lhs_f32.size(); ++i) {
        lhs_f32[i] = static_cast<float>(lhs[i]);
    }
    for (size_t i = 0; i < rhs_f32.size(); ++i) {
        rhs_f32[i] = static_cast<float>(rhs[i]);
    }
...
void cactus_conv2d_pointwise_f16_1x1_nchw_gemm(
    ...
    CactusThreading::parallel_for(
        ...
        [&](size_t n_start, size_t n_end) {
            ...
            std::vector<float> packed_out(HW * C_out);
            ...
            for (size_t n = n_start; n < n_end; ++n) {
                ...
                fp16_rowmajor_gemm_rhs_transposed_to_fp32(
                    packed_in.data(),
                    weight,
                    packed_out.data(),
                    HW,
                ...
                for (size_t hw = 0; hw < HW; ++hw) {
                    const size_t h = hw / W;
                    const size_t w = hw - h * W;
                    const float* row = packed_out.data() + hw * C_out;
                    for (size_t oc = 0; oc < C_out; ++oc) {
                        float outv = row[oc];
...
void cactus_conv1d_pointwise_f16_gemm(
    ...
    std::vector<float> output_f32(M * C_out);
    fp16_rowmajor_gemm_rhs_transposed_to_fp32(
        input,
        weight,
        output_f32.data(),
    ...
    CactusThreading::parallel_for(
        ...
        [&](size_t row_start, size_t row_end) {
            for (size_t m = row_start; m < row_end; ++m) {
                const float* src_row = output_f32.data() + m * C_out;
                __fp16* dst_row = output + m * C_out;
                for (size_t oc = 0; oc < C_out; ++oc) {
                    float value = src_row[oc];
                    if (bias) {
                        value += static_cast<float>(bias[oc]);
                    }
                    dst_row[oc] = static_cast<__fp16>(value);
                }
            }
        }
    );
