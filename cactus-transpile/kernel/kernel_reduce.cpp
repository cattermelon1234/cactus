...
template<typename FinalizeFn>
static void axis_reduce_f32_impl(const float* input, float* output,
                                 size_t outer_size, size_t axis_size, size_t inner_size,
                                 FinalizeFn finalize) {
    CactusThreading::parallel_for_2d(
        ...
        [&](size_t outer, size_t inner) {
            ...
            for (size_t a = 0; a < axis_size; ++a) {
                total += input[outer * axis_size * inner_size + a * inner_size + inner];
            }
            output[outer * inner_size + inner] = finalize(total, axis_size);
        }
    );
}
...
template<typename ScalarReduceOp>
static void axis_reduce_f32_impl(const float* input, float* output,
                                 size_t outer_size, size_t axis_size, size_t inner_size,
                                 float init_scalar,
...
double cactus_mean_all_f32(const float* data, size_t num_elements) {
    return cactus_sum_all_f32(data, num_elements) / static_cast<double>(num_elements);
}
...
void cactus_variance_axis_f32(const float* input, float* output, size_t outer_size, size_t axis_size, size_t inner_size) {
    CactusThreading::parallel_for_2d(
        ...
        [&](size_t outer, size_t inner) {
            ...
            float sum_sq = 0.0f;
            for (size_t a = 0; a < axis_size; ++a) {
                float x = input[outer * axis_size * inner_size + a * inner_size + inner];
                sum += x;
                sum_sq += x * x;
            }
            float mean = sum / static_cast<float>(axis_size);
            float mean_sq = sum_sq / static_cast<float>(axis_size);
            output[outer * inner_size + inner] = mean_sq - mean * mean;
        }
    );
}
...
void cactus_cumsum_axis_f16(const __fp16* input, __fp16* output, size_t outer_size, size_t axis_size, size_t inner_size) {
    CactusThreading::parallel_for_2d(
        ...
        [&](size_t outer, size_t inner) {
            float running = 0.0f;
            for (size_t a = 0; a < axis_size; ++a) {
                const size_t offset = outer * axis_size * inner_size + a * inner_size + inner;
                running += static_cast<float>(input[offset]);
                output[offset] = static_cast<__fp16>(running);
            }
        }
    );
}

void cactus_cumsum_axis_f32(const float* input, float* output, size_t outer_size, size_t axis_size, size_t inner_size) {
    CactusThreading::parallel_for_2d(
        outer_size,
        inner_size,
        CactusThreading::Thresholds::AXIS_REDUCE,
        [&](size_t outer, size_t inner) {
            ...
            for (size_t a = 0; a < axis_size; ++a) {
                const size_t offset = outer * axis_size * inner_size + a * inner_size + inner;
                running += input[offset];
                output[offset] = running;
            }
        }
    );
}
