...
inline size_t sliding_window_kv_start(size_t absolute_q_pos, size_t window_size) {
    if (window_size == 0) {
        return 0;
    }
    const size_t visible_tokens = absolute_q_pos + 1;
    if (visible_tokens <= window_size) {
        return 0;
    }
    return visible_tokens - window_size;
}

inline bool sliding_window_masks_kv(size_t absolute_q_pos, size_t kv_pos, size_t window_size) {
    return window_size > 0 && kv_pos < absolute_q_pos && (absolute_q_pos - kv_pos) >= window_size;
}
...
static inline void cactus_attention_f16_fast(
    ...
    CactusThreading::parallel_for(batch_size * num_q_heads * seq_len, CactusThreading::Thresholds::ATTENTION,
        ...
        for (size_t work = start; work < end; ++work) {
            ...
            size_t kv_start = sliding_window_kv_start(abs_q, window_size);
            ...
            for (size_t work_idx = start_idx; work_idx < end_idx; ++work_idx) {
                ...
                const __fp16* M = mask ? (mask + batch_idx * mask_batch_stride) : nullptr;
                    ...
                    if (window_size > 0 && window_size < kv_seq_len) {
                        kv_start = sliding_window_kv_start(absolute_q_pos, window_size);
                        if (is_causal) {
                    ...
                    for (size_t kv_block_start = kv_start; kv_block_start < kv_end; kv_block_start += BLOCK_SIZE) {
                        ...
                        for (size_t kv_idx = 0; kv_idx < block_size; ++kv_idx) {
                            ...
                            else if (sliding_window_masks_kv(absolute_q_pos, kv_pos, window_size)) {
                                score = NEG_INF;
                            }
                            else if (M) {
    ...
    CactusThreading::parallel_for(batch_size * num_q_heads * seq_len, CactusThreading::Thresholds::ATTENTION,
        [=](size_t start_idx, size_t end_idx) {
            ...
            for (size_t work_idx = start_idx; work_idx < end_idx; ++work_idx) {
                ...
                size_t kv_start = sliding_window_kv_start(absolute_q_pos, window_size);
