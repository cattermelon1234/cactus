...
int cactus_decode_tokens(
    ...
    if (!model || !tokens || token_len == 0 || !out_token_id) {
    ...
    try {
        ...
        handle->model->reset_cache();
        *out_token_id = handle->model->decode(vec, temperature, top_p, top_k);
        return 0;
    } catch (const std::exception& e) {
        last_error_message = e.what();
        return -1;
    } catch (...) {
        last_error_message = "Unknown error during raw token decode";
        return -1;
    }
}
