...
cactus_model_t cactus_init(const char* model_path, const char* corpus_dir, bool cache_index) {
    ...
    try {
        ...
        if (model_type == Config::ModelType::WHISPER ||
            ...
            const std::filesystem::path vad_config_path = std::filesystem::path(vad_path) / "config.txt";
            if (std::filesystem::exists(vad_config_path)) {
                ...
                CACTUS_LOG_WARN("init", "VAD weights not found at " << vad_path << "; continuing without VAD");
            }
        }
