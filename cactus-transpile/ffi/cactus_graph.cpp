...
int cactus_graph_not_equal(cactus_graph_t graph, cactus_node_t a, cactus_node_t b, cactus_node_t* out) {
    if (!graph || !out) return fail_invalid("Invalid args to cactus_graph_not_equal");
    try {
...
int cactus_graph_scalar_floor_divide(cactus_graph_t graph, cactus_node_t x, float value, cactus_node_t *out) {
    if (!graph || !out) {
        last_error_message = "Invalid args to cactus_graph_scalar_floor_divide";
        return -1;
    }
    try {
...
int cactus_graph_scalar_not_equal(cactus_graph_t graph, cactus_node_t x, float value, cactus_node_t* out) {
    if (!graph || !out) return fail_invalid("Invalid args to cactus_graph_scalar_not_equal");
    try {
...
int cactus_graph_masked_select_prefix(cactus_graph_t graph, cactus_node_t input, cactus_node_t mask, cactus_node_t* out) {
    if (!graph || !out) return fail_invalid("Invalid args to cactus_graph_masked_select_prefix");
    try {
        *out = static_cast<cactus_node_t>(
            as_graph(graph)->graph.masked_select_prefix(static_cast<size_t>(input), static_cast<size_t>(mask))
        );
        return 0;
    } catch (const std::exception& e) {
        last_error_message = e.what();
...
int cactus_graph_masked_scatter(
    cactus_graph_t graph,
    cactus_node_t input,
    cactus_node_t mask,
    cactus_node_t source,
    cactus_node_t* out
) {
    if (!graph || !out) return fail_invalid("Invalid args to cactus_graph_masked_scatter");
    try {
        *out = static_cast<cactus_node_t>(
            as_graph(graph)->graph.masked_scatter(
...
int cactus_graph_matmul(
    ...
    try {
        *out = static_cast<cactus_node_t>(
            as_graph(graph)->graph.matmul(
                static_cast<size_t>(a),
                static_cast<size_t>(b),
                pretransposed_rhs,
                static_cast<ComputeBackend>(backend),
                static_cast<Precision>(output_precision)
            )
        );
        return 0;
    } catch (const std::exception& e) {
...
int cactus_graph_gather(cactus_graph_t graph, cactus_node_t tensor, cactus_node_t indices, int32_t axis, cactus_node_t* out) {
    if (!graph || !out) return fail_invalid("Invalid args to cactus_graph_gather");
    try {
        *out = static_cast<cactus_node_t>(as_graph(graph)->graph.gather(
...
int cactus_graph_conv2d(
    cactus_graph_t graph,
    cactus_node_t input,
    cactus_node_t weight,
    bool has_bias,
    cactus_node_t bias,
    size_t stride,
    size_t padding,
    ...
    if (!graph || !out) return fail_invalid("Invalid args to cactus_graph_conv2d");
    try {
        if (has_bias) {
            *out = static_cast<cactus_node_t>(
                as_graph(graph)->graph.conv2d(
                    static_cast<size_t>(input),
                    static_cast<size_t>(weight),
                    static_cast<size_t>(bias),
                    stride,
                    padding,
                    dilation,
                    groups
