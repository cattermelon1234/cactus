...
size_t conv2d_output_dim(size_t input, size_t kernel, size_t stride, size_t padding, size_t dilation) {
    if (stride == 0) {
    ...
    const size_t effective_kernel = dilation * (kernel - 1) + 1;
    if (input + 2 * padding < effective_kernel) {
...
void validate_conv2d_weight_shape(
    const BufferDesc& xin,
    const BufferDesc& w,
    size_t groups,
    const char* op_name
) {
    if (xin.shape.size() != 4) {
        throw std::runtime_error(std::string(op_name) + " expects input [N, C_in, H, W]");
    }
    if (w.shape.size() != 4) {
        throw std::runtime_error(std::string(op_name) + " weight must be [C_out, C_in/groups, K_h, K_w]");
    }
    if (groups == 0) {
        throw std::runtime_error(std::string(op_name) + " groups must be > 0");
    }
    ...
    if ((c_in % groups) != 0) {
        throw std::runtime_error(std::string(op_name) + " input channels must be divisible by groups");
    }
    if ((c_out % groups) != 0) {
        throw std::runtime_error(std::string(op_name) + " output channels must be divisible by groups");
    }
    if (c_in_per_group * groups != c_in) {
        throw std::runtime_error(std::string(op_name) + " weight must match [C_out, C_in/groups, K_h, K_w]");
    }
    if (xin.shape[2] == 0 || xin.shape[3] == 0) {
        throw std::runtime_error(std::string(op_name) + " input spatial dimensions must be > 0");
    }
}

int normalize_reduction_axis(const BufferDesc& input_buffer, int axis, const char* op_name) {
    if (input_buffer.shape.empty()) {
        if (axis == 0 || axis == -1) {
            return 0;
        }
        throw std::runtime_error(std::string(op_name) + " axis out of range for scalar input");
    }
    ...
    if (actual_axis < 0 || static_cast<size_t>(actual_axis) >= input_buffer.shape.size()) {
        throw std::runtime_error(std::string(op_name) + " axis out of range");
    }
    return actual_axis;
}

std::vector<size_t> reduction_output_shape(const BufferDesc& input_buffer, int axis) {
...
size_t CactusGraph::expand(size_t input, const std::vector<size_t>& new_shape) {
    ...
    if (new_shape.size() < input_shape.size()) {
        throw std::invalid_argument("expand cannot reduce rank");
    }

    const size_t rank_delta = new_shape.size() - input_shape.size();
    for (size_t axis = 0; axis < new_shape.size(); ++axis) {
        const size_t input_dim = axis < rank_delta ? 1 : input_shape[axis - rank_delta];
        const size_t output_dim = new_shape[axis];
        if (input_dim != 1 && input_dim != output_dim) {
            throw std::invalid_argument("expand target shape is not broadcast-compatible with input");
        }
    }
...
size_t CactusGraph::masked_select_prefix(size_t input, size_t mask) {
    ...
    if (mask_buffer.shape.empty()) {
        throw std::invalid_argument("masked_select_prefix requires a non-scalar mask");
    }
    if (input_buffer.shape.size() < mask_buffer.shape.size()) {
        throw std::invalid_argument("masked_select_prefix mask rank must not exceed input rank");
    }
    for (size_t i = 0; i < mask_buffer.shape.size(); ++i) {
        if (input_buffer.shape[i] != mask_buffer.shape[i]) {
            throw std::invalid_argument("masked_select_prefix mask shape must match the input prefix shape");
        }
    }

    std::vector<size_t> output_shape;
    output_shape.push_back(mask_buffer.total_size);
    for (size_t i = mask_buffer.shape.size(); i < input_buffer.shape.size(); ++i) {
        output_shape.push_back(input_buffer.shape[i]);
    }

    OpParams params{};
    params.output_precision = input_buffer.precision;
    return add_node(OpType::MASKED_SELECT_PREFIX, {input, mask}, output_shape, params);
}

size_t CactusGraph::masked_scatter(size_t input, size_t mask, size_t source) {
    const auto& input_buffer = get_output_buffer(input);
    const auto& mask_buffer = get_output_buffer(mask);
    const auto& source_buffer = get_output_buffer(source);

    if (input_buffer.shape != mask_buffer.shape) {
        throw std::invalid_argument("masked_scatter mask shape must match input shape");
    }
    if (input_buffer.precision != source_buffer.precision) {
        throw std::invalid_argument("masked_scatter source precision must match input precision");
    }

    OpParams params{};
    params.output_precision = input_buffer.precision;
    return add_node(OpType::MASKED_SCATTER, {input, mask, source}, input_buffer.shape, params);
}

size_t CactusGraph::sum(size_t input, int axis) {
    const auto& input_buffer = get_output_buffer(input);
    const int actual_axis = normalize_reduction_axis(input_buffer, axis, "sum");
    const std::vector<size_t> output_shape = reduction_output_shape(input_buffer, actual_axis);
    OpParams params{.axis = actual_axis, .output_precision = input_buffer.precision};
    return add_node(OpType::SUM, {input}, output_shape, params);
}

size_t CactusGraph::mean(size_t input, int axis) {
    const auto& input_buffer = get_output_buffer(input);
    const int actual_axis = normalize_reduction_axis(input_buffer, axis, "mean");
    const std::vector<size_t> output_shape = reduction_output_shape(input_buffer, actual_axis);
    OpParams params{.axis = actual_axis, .output_precision = input_buffer.precision};
...
size_t CactusGraph::variance(size_t input, int axis) {
    const auto& input_buffer = get_output_buffer(input);
    const int actual_axis = normalize_reduction_axis(input_buffer, axis, "variance");
    const std::vector<size_t> output_shape = reduction_output_shape(input_buffer, actual_axis);
    OpParams params{.axis = actual_axis, .output_precision = input_buffer.precision};
...
size_t CactusGraph::min(size_t input, int axis) {
    const auto& input_buffer = get_output_buffer(input);
    const int actual_axis = normalize_reduction_axis(input_buffer, axis, "min");
    const std::vector<size_t> output_shape = reduction_output_shape(input_buffer, actual_axis);
    OpParams params{.axis = actual_axis, .output_precision = input_buffer.precision};
...
size_t CactusGraph::max(size_t input, int axis) {
    const auto& input_buffer = get_output_buffer(input);
    const int actual_axis = normalize_reduction_axis(input_buffer, axis, "max");
    const std::vector<size_t> output_shape = reduction_output_shape(input_buffer, actual_axis);
    OpParams params{.axis = actual_axis, .output_precision = input_buffer.precision};
...
size_t CactusGraph::cumsum(size_t input, int axis) {
    const auto& input_buffer = get_output_buffer(input);
    const int actual_axis = normalize_reduction_axis(input_buffer, axis, "cumsum");
    OpParams params{.axis = actual_axis, .output_precision = input_buffer.precision};
    return add_node(OpType::CUMSUM, {input}, input_buffer.shape, params);
}
...
size_t CactusGraph::conv2d(size_t input, size_t weight, size_t stride, size_t padding, size_t dilation, size_t groups) {
    const auto& xin = get_output_buffer(input);
    const auto& w = get_output_buffer(weight);

    validate_conv2d_weight_shape(xin, w, groups, "conv2d");

    const size_t n = xin.shape[0];
    const size_t c_out = w.shape[0];
    const size_t h_out = conv2d_output_dim(xin.shape[2], w.shape[2], stride, padding, dilation);
    const size_t w_out = conv2d_output_dim(xin.shape[3], w.shape[3], stride, padding, dilation);
...
size_t CactusGraph::conv2d(size_t input, size_t weight, size_t bias, size_t stride, size_t padding, size_t dilation, size_t groups) {
    const auto& xin = get_output_buffer(input);
    const auto& w = get_output_buffer(weight);
    const auto& b = get_output_buffer(bias);
...
size_t CactusGraph::scalar_floor_divide(size_t input, float value) {
    OpParams params{};
    params.scalar = value;
    params.output_precision = get_output_buffer(input).precision;
    return add_node(OpType::SCALAR_FLOOR_DIVIDE, {input}, {}, params);
}
...
size_t CactusGraph::gather(size_t tensor, size_t indices, int axis) {
    ...
    int actual_axis = axis;
    if (actual_axis < 0) {
        actual_axis += static_cast<int>(tensor_buffer.shape.size());
    }
    ...
    if (idx_shape.size() == tensor_buffer.shape.size()) {
        ...
        if (actual_axis != 0) {
            throw std::runtime_error("Lower-rank gather only supports axis 0");
        }
        output_shape = idx_shape;
