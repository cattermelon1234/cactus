...
static float dispatch_binary_op_scalar_fp32(OpType op, float lhs, float rhs) {
    switch (op) {
        case OpType::ADD:
            return lhs + rhs;
        case OpType::ADD_CLIPPED: {
            ...
            float value = lhs + rhs;
            return std::fmin(std::fmax(value, -FP16_MAX), FP16_MAX);
        }
        case OpType::SUBTRACT:
            return lhs - rhs;
        case OpType::MULTIPLY:
        ...
        default:
            throw std::runtime_error("unsupported FP32 binary op");
    }
}

static void dispatch_binary_op_fp32(OpType op, const float* lhs, const float* rhs, float* output, size_t count) {
    for (size_t i = 0; i < count; ++i) {
        output[i] = dispatch_binary_op_scalar_fp32(op, lhs[i], rhs[i]);
    }
}

static void dispatch_binary_op_fp32_to_fp16(
    OpType op,
    const float* lhs,
    const float* rhs,
    __fp16* output,
    ...
    for (size_t i = 0; i < count; ++i) {
        output[i] = static_cast<__fp16>(dispatch_binary_op_scalar_fp32(op, lhs[i], rhs[i]));
    }
}

static void dispatch_binary_op_f16_to_fp32(
    OpType op,
    const __fp16* lhs,
    const __fp16* rhs,
    float* output,
    ...
    for (size_t i = 0; i < count; ++i) {
        output[i] = dispatch_binary_op_scalar_fp32(
            op,
...
static void dispatch_binary_op_broadcast_fp32(
    OpType op,
    const float* lhs,
    const float* rhs,
    float* output,
    ...
    for (size_t linear_index = 0; linear_index < output_size; ++linear_index) {
        size_t remainder = linear_index;
        size_t lhs_index = 0;
        size_t rhs_index = 0;
        for (size_t dim = output_shape.size(); dim-- > 0;) {
            const size_t coord = remainder % output_shape[dim];
            remainder /= output_shape[dim];
            lhs_index += coord * lhs_strides[dim];
            rhs_index += coord * rhs_strides[dim];
        }
        output[linear_index] = dispatch_binary_op_scalar_fp32(op, lhs[lhs_index], rhs[rhs_index]);
    }
}

static void dispatch_binary_op_broadcast_fp32_to_fp16(
    OpType op,
    const float* lhs,
    const float* rhs,
    __fp16* output,
    ...
    for (size_t linear_index = 0; linear_index < output_size; ++linear_index) {
        ...
        output[linear_index] = static_cast<__fp16>(
            dispatch_binary_op_scalar_fp32(op, lhs[lhs_index], rhs[rhs_index])
        );
    }
}

static void dispatch_binary_op_broadcast_f16_to_fp32(
    OpType op,
    const __fp16* lhs,
    const __fp16* rhs,
    float* output,
    ...
    for (size_t linear_index = 0; linear_index < output_size; ++linear_index) {
        ...
        output[linear_index] = dispatch_binary_op_scalar_fp32(
            op,
            static_cast<float>(lhs[lhs_index]),
...
void dispatch_unary_op_f16(OpType op, const __fp16* input, __fp16* output, size_t count, float param) {
    ...
    switch (op) {
        ...
        case OpType::SCALAR_FLOOR_DIVIDE: scalar_op = ScalarOpType::FLOOR_DIVIDE; break;
...
static float dispatch_unary_op_scalar_fp32(OpType op, float input, float param) {
    switch (op) {
        ...
        case OpType::SCALAR_DIVIDE:
            return input / param;
        case OpType::SCALAR_NOT_EQUAL:
            return input != param ? 1.0f : 0.0f;
        ...
        case OpType::SCALAR_FLOOR_DIVIDE:
            return std::floor(input / param);
        case OpType::ABS:
            return std::fabs(input);
        case OpType::POW:
            return std::pow(input, param);
        default:
            throw std::runtime_error("unsupported FP32 unary op");
    }
}

static void dispatch_unary_op_fp32(OpType op, const float* input, float* output, size_t count, float param) {
    for (size_t i = 0; i < count; ++i) {
        output[i] = dispatch_unary_op_scalar_fp32(op, input[i], param);
    }
}

static void dispatch_unary_op_fp32_to_fp16(
    OpType op,
    const float* input,
    __fp16* output,
    size_t count,
    ...
    for (size_t i = 0; i < count; ++i) {
        output[i] = static_cast<__fp16>(dispatch_unary_op_scalar_fp32(op, input[i], param));
    }
}

static void dispatch_unary_op_f16_to_fp32(
    OpType op,
    const __fp16* input,
    float* output,
    size_t count,
    ...
    for (size_t i = 0; i < count; ++i) {
        output[i] = dispatch_unary_op_scalar_fp32(op, static_cast<float>(input[i]), param);
    }
}
...
void compute_binary_op_node(GraphNode& node, const std::vector<std::unique_ptr<GraphNode>>& nodes, const std::unordered_map<size_t, size_t>& node_index_map) {
    ...
    const Precision output_precision = node.output_buffer.precision;

    if (lhs.precision != rhs.precision) {
        throw std::runtime_error("Binary operations require matching input precision");
    }

    if (lhs.precision != Precision::FP16 && lhs.precision != Precision::FP32) {
        throw std::runtime_error(
            "Binary operations only support FP16/FP32 precision (got " +
    ...
    if (node.params.broadcast_info.needs_broadcasting) {
        ...
        if (lhs.precision == Precision::FP16) {
            ...
            } else if (output_precision == Precision::FP32) {
                dispatch_binary_op_broadcast_f16_to_fp32(
                    node.op_type,
                    lhs.data_as<__fp16>(),
                    rhs.data_as<__fp16>(),
                    ...
                    lhs_strides,
                    rhs_strides,
                    node.params.broadcast_info.output_shape
                );
            } else {
                throw std::runtime_error("Binary broadcast FP16 inputs only support FP16/FP32 outputs");
            }
        } else {
            if (output_precision == Precision::FP32) {
                dispatch_binary_op_broadcast_fp32(
                    node.op_type,
                    lhs.data_as<float>(),
                    rhs.data_as<float>(),
                    node.output_buffer.data_as<float>(),
            ...
            } else if (output_precision == Precision::FP16) {
                dispatch_binary_op_broadcast_fp32_to_fp16(
                    node.op_type,
                    lhs.data_as<float>(),
                    rhs.data_as<float>(),
                    node.output_buffer.data_as<__fp16>(),
                ...
                throw std::runtime_error("Binary broadcast FP32 inputs only support FP16/FP32 outputs");
            }
        }
    } else {
        if (lhs.precision == Precision::FP16) {
            if (output_precision == Precision::FP16) {
                ...
                dispatch_binary_op_f16_to_fp32(
                    node.op_type,
                    lhs.data_as<__fp16>(),
                    rhs.data_as<__fp16>(),
                    node.output_buffer.data_as<float>(),
                ...
                throw std::runtime_error("Binary FP16 inputs only support FP16/FP32 outputs");
            }
        } else {
            if (output_precision == Precision::FP32) {
                dispatch_binary_op_fp32(node.op_type, lhs.data_as<float>(),
                                        rhs.data_as<float>(), node.output_buffer.data_as<float>(),
                                        node.output_buffer.total_size);
            } else if (output_precision == Precision::FP16) {
                dispatch_binary_op_fp32_to_fp16(
                    node.op_type,
                    lhs.data_as<float>(),
                    rhs.data_as<float>(),
                    node.output_buffer.data_as<__fp16>(),
                ...
                throw std::runtime_error("Binary FP32 inputs only support FP16/FP32 outputs");
            }
        }
    }
}
...
void compute_unary_op_node(GraphNode& node, const std::vector<std::unique_ptr<GraphNode>>& nodes, const std::unordered_map<size_t, size_t>& node_index_map) {
    ...
    if (input.precision == Precision::FP16) {
        if (output_precision == Precision::FP16) {
            ...
            throw std::runtime_error("Scalar FP16 inputs only support FP16/FP32 outputs");
        }
    } else {
        if (output_precision == Precision::FP32) {
            dispatch_unary_op_fp32(node.op_type, input.data_as<float>(),
                                   node.output_buffer.data_as<float>(),
                                   node.output_buffer.total_size, node.params.scalar);
        } else if (output_precision == Precision::FP16) {
            dispatch_unary_op_fp32_to_fp16(
                node.op_type,
                input.data_as<float>(),
                node.output_buffer.data_as<__fp16>(),
                node.output_buffer.total_size,
            ...
            throw std::runtime_error("Scalar FP32 inputs only support FP16/FP32 outputs");
        }
    }
}
...
void compute_reduce_node(GraphNode& node, const std::vector<std::unique_ptr<GraphNode>>& nodes, const std::unordered_map<size_t, size_t>& node_index_map) {
    ...
    if (input_buffer.precision != Precision::FP16 && input_buffer.precision != Precision::FP32) {
        throw std::runtime_error("Reduction operations only support FP16 and FP32 precision");
    }
    ...
    if (axis == -1) {
        if (input_buffer.precision == Precision::FP32) {
            switch (node.op_type) {
                ...
                case OpType::MIN:
                    node.output_buffer.data_as<float>()[0] = cactus_min_all_f32(input_buffer.data_as<float>(), input_buffer.total_size);
                    break;
                case OpType::MAX:
                    node.output_buffer.data_as<float>()[0] = cactus_max_all_f32(input_buffer.data_as<float>(), input_buffer.total_size);
                    break;
                default:
                    break;
            }
