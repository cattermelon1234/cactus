...
template <typename T>
static void transpose_scalar_nd(
    const T* source,
    T* destination,
    ...
    std::vector<size_t> output_shape(ndim, 0);
    for (size_t axis = 0; axis < ndim; ++axis) {
        output_shape[axis] = shape[permutation[axis]];
    }

    std::vector<size_t> input_strides(ndim, 1);
    for (size_t axis = ndim; axis-- > 1;) {
        input_strides[axis - 1] = input_strides[axis] * shape[axis];
    }

    std::vector<size_t> output_strides(ndim, 1);
    for (size_t axis = ndim; axis-- > 1;) {
    ...
    for (size_t out_idx = 0; out_idx < total; ++out_idx) {
        ...
        size_t src_idx = 0;
        for (size_t axis = 0; axis < ndim; ++axis) {
            const size_t coord = remaining / output_strides[axis];
            remaining %= output_strides[axis];
            src_idx += coord * input_strides[permutation[axis]];
        }
        destination[out_idx] = source[src_idx];
    }
}
...
void compute_transpose_node(GraphNode& node, const std::vector<std::unique_ptr<GraphNode>>& nodes, const std::unordered_map<size_t, size_t>& node_index_map) {
    ...
    if (input_buffer.precision == Precision::FP16) {
        ...
        cactus_transpose_f16(
            input,
            output,
            input_buffer.shape.data(),
            permutation.data(),
            permutation.size(),
            0,
            input_buffer.total_size
        );
        return;
    }
    ...
    if (input_buffer.precision == Precision::FP32) {
        ...
        transpose_scalar_nd<float>(input, output, input_buffer.shape, permutation);
        return;
    }
...
void compute_gather_node(GraphNode& node, const std::vector<std::unique_ptr<GraphNode>>& nodes, const std::unordered_map<size_t, size_t>& node_index_map) {
    ...
    if (indices_buffer.shape.size() == tensor_buffer.shape.size()) {
        if (tensor_buffer.precision == Precision::INT4) {
            throw std::runtime_error("Generic gather does not support INT4 tensors");
        }

        const size_t rank = tensor_buffer.shape.size();
        if (rank == 0) {
            throw std::runtime_error("Cannot gather from scalar tensor");
        }

        std::vector<size_t> input_strides(rank, 1);
        std::vector<size_t> output_strides(rank, 1);
        for (size_t i = rank; i-- > 1;) {
            input_strides[i - 1] = input_strides[i] * tensor_buffer.shape[i];
            output_strides[i - 1] = output_strides[i] * indices_buffer.shape[i];
        }

        auto read_index = [&](size_t flat_index) -> size_t {
            if (indices_buffer.precision == Precision::INT8) {
                const int8_t* indices = indices_buffer.data_as<int8_t>();
                return static_cast<size_t>(indices[flat_index]);
            }
            if (indices_buffer.precision == Precision::FP16) {
                const __fp16* indices = indices_buffer.data_as<__fp16>();
                return static_cast<size_t>(indices[flat_index]);
            }
        ...
        const size_t total = node.output_buffer.total_size;
        const size_t bytes_per_scalar = PrecisionTraits::packed_size_of(tensor_buffer.precision, 1);
        const char* input_data = static_cast<const char*>(tensor_buffer.get_data());
        char* output_data = static_cast<char*>(node.output_buffer.get_data());

        for (size_t flat_output = 0; flat_output < total; ++flat_output) {
            size_t remainder = flat_output;
            size_t input_offset = 0;

            const size_t gathered_index = read_index(flat_output);
            if (gathered_index >= tensor_buffer.shape[static_cast<size_t>(axis)]) {
                throw std::runtime_error(
                    "Gather index " + std::to_string(gathered_index) +
                    " out of bounds for dimension " + std::to_string(tensor_buffer.shape[static_cast<size_t>(axis)])
                );
            }

            remainder = flat_output;
            for (size_t dim = 0; dim < rank; ++dim) {
                const size_t coord = remainder / output_strides[dim];
                remainder %= output_strides[dim];
                const size_t input_coord = (static_cast<int>(dim) == axis) ? gathered_index : coord;
                input_offset += input_coord * input_strides[dim];
            }

            std::memcpy(
                output_data + flat_output * bytes_per_scalar,
                input_data + PrecisionTraits::byte_offset_of(tensor_buffer.precision, input_offset),
                bytes_per_scalar
            );
        }
        return;
...
static size_t product_range(const std::vector<size_t>& shape, size_t start, size_t end) {
    size_t product = 1;
...
static void concat_same_precision_tensors(
    const std::vector<const BufferDesc*>& inputs,
    BufferDesc& output,
    int axis
) {
    if (inputs.empty()) {
        throw std::runtime_error("concat requires at least one input tensor");
    }
    if (axis < 0) {
        throw std::runtime_error("concat does not support negative axis");
    }

    const auto& first = *inputs.front();
    if (first.precision == Precision::INT4 || first.is_grouped_int8()) {
        throw std::runtime_error("concat does not support packed/grouped integer tensors");
    }
    ...
    const size_t element_bytes = PrecisionTraits::size_of(first.precision);
    const size_t outer = product_range(first.shape, 0, static_cast<size_t>(axis));
    const size_t inner = product_range(first.shape, static_cast<size_t>(axis) + 1, rank);
    const size_t output_axis = output.shape[static_cast<size_t>(axis)];
    char* output_ptr = static_cast<char*>(output.get_data());

    for (size_t outer_idx = 0; outer_idx < outer; ++outer_idx) {
        size_t output_axis_offset = 0;
        for (const BufferDesc* input : inputs) {
            if (input->precision != first.precision) {
                throw std::runtime_error("concat requires all input precisions to match");
            }
            if (input->shape.size() != rank) {
                throw std::runtime_error("concat requires all input ranks to match");
            }
            if (input->precision == Precision::INT4 || input->is_grouped_int8()) {
                throw std::runtime_error("concat does not support packed/grouped integer tensors");
            }

            const size_t axis_size = input->shape[static_cast<size_t>(axis)];
            const size_t chunk_elements = axis_size * inner;
            const size_t output_base = ((outer_idx * output_axis) + output_axis_offset) * inner * element_bytes;
            const size_t input_base = outer_idx * chunk_elements * element_bytes;
            std::memcpy(
                output_ptr + output_base,
                static_cast<const char*>(input->get_data()) + input_base,
                chunk_elements * element_bytes
            );
            output_axis_offset += axis_size;
        }
    }
}
...
void compute_concat_node(GraphNode& node, const std::vector<std::unique_ptr<GraphNode>>& nodes, const std::unordered_map<size_t, size_t>& node_index_map) {
    ...
    std::vector<const BufferDesc*> inputs = {&input1_buffer, &input2_buffer};
    concat_same_precision_tensors(inputs, node.output_buffer, node.params.axis);
}
...
static bool mask_element_is_true(const BufferDesc& mask_buffer, size_t index) {
    switch (mask_buffer.precision) {
        case Precision::FP16:
            return static_cast<float>(mask_buffer.data_as<__fp16>()[index]) != 0.0f;
        case Precision::FP32:
            return mask_buffer.data_as<float>()[index] != 0.0f;
        case Precision::INT8:
            return mask_buffer.data_as<int8_t>()[index] != 0;
        default:
            throw std::runtime_error("Unsupported mask precision for masked select/scatter");
    }
}

void compute_masked_select_prefix_node(
    GraphNode& node,
    const std::vector<std::unique_ptr<GraphNode>>& nodes,
    const std::unordered_map<size_t, size_t>& node_index_map
) {
    ...
    if (input_buffer.precision == Precision::INT4 || input_buffer.is_grouped_int8()) {
        throw std::runtime_error("masked_select_prefix does not support packed/grouped integer tensors");
    }
    if (mask_buffer.shape.empty()) {
        throw std::runtime_error("masked_select_prefix requires non-scalar mask");
    }
    if (input_buffer.shape.size() < mask_buffer.shape.size()) {
        throw std::runtime_error("masked_select_prefix mask rank must not exceed input rank");
    }
    for (size_t i = 0; i < mask_buffer.shape.size(); ++i) {
        if (input_buffer.shape[i] != mask_buffer.shape[i]) {
            throw std::runtime_error("masked_select_prefix mask shape must match the input prefix shape");
        }
    }

    const size_t prefix_elems = mask_buffer.total_size;
    const size_t trailing_elems = input_buffer.total_size / prefix_elems;
    const size_t block_bytes = trailing_elems * PrecisionTraits::size_of(input_buffer.precision);
    const char* input_ptr = static_cast<const char*>(input_buffer.get_data());
    char* output_ptr = static_cast<char*>(node.output_buffer.get_data());
    std::memset(output_ptr, 0, node.output_buffer.byte_size);

    size_t packed_row = 0;
    for (size_t prefix_idx = 0; prefix_idx < prefix_elems; ++prefix_idx) {
        if (!mask_element_is_true(mask_buffer, prefix_idx)) {
            continue;
        }
        std::memcpy(
            output_ptr + (packed_row * block_bytes),
            input_ptr + (prefix_idx * block_bytes),
            block_bytes
        );
        packed_row++;
    }
...
void compute_masked_scatter_node(
    GraphNode& node,
    const std::vector<std::unique_ptr<GraphNode>>& nodes,
    const std::unordered_map<size_t, size_t>& node_index_map
) {
    const auto& input_buffer = get_input(node, 0, nodes, node_index_map);
    const auto& mask_buffer = get_input(node, 1, nodes, node_index_map);
    const auto& source_buffer = get_input(node, 2, nodes, node_index_map);

    if (input_buffer.precision == Precision::INT4 || source_buffer.precision == Precision::INT4) {
        throw std::runtime_error("masked_scatter does not support INT4 tensors");
    }
    if (input_buffer.shape != mask_buffer.shape) {
        throw std::runtime_error("masked_scatter mask shape must match input shape");
    }
    if (input_buffer.precision != source_buffer.precision) {
        throw std::runtime_error("masked_scatter source precision must match input precision");
    }

    const size_t element_bytes = PrecisionTraits::size_of(input_buffer.precision);
    const char* input_ptr = static_cast<const char*>(input_buffer.get_data());
    const char* source_ptr = static_cast<const char*>(source_buffer.get_data());
    char* output_ptr = static_cast<char*>(node.output_buffer.get_data());
    std::memcpy(output_ptr, input_ptr, node.output_buffer.byte_size);

    size_t source_index = 0;
    for (size_t element_idx = 0; element_idx < mask_buffer.total_size; ++element_idx) {
        if (!mask_element_is_true(mask_buffer, element_idx)) {
            continue;
        }
        if (source_index >= source_buffer.total_size) {
            throw std::runtime_error("masked_scatter source does not contain enough elements");
        }
        std::memcpy(
            output_ptr + (element_idx * element_bytes),
            source_ptr + (source_index * element_bytes),
            element_bytes
        );
        source_index++;
    }
...
void compute_expand_node(
    ...
    if (input_buffer.precision == Precision::INT4 || input_buffer.is_grouped_int8()) {
        throw std::runtime_error("expand does not support packed/grouped integer tensors");
    }
    if (output_shape.size() < input_shape.size()) {
        throw std::runtime_error("expand cannot reduce rank");
    }

    const size_t rank_delta = output_shape.size() - input_shape.size();
    std::vector<size_t> padded_input_shape(output_shape.size(), 1);
    for (size_t axis = 0; axis < input_shape.size(); ++axis) {
        padded_input_shape[rank_delta + axis] = input_shape[axis];
    }
    for (size_t axis = 0; axis < output_shape.size(); ++axis) {
        if (padded_input_shape[axis] != 1 && padded_input_shape[axis] != output_shape[axis]) {
            throw std::runtime_error("expand target shape is not broadcast-compatible with input");
        }
    }

    std::vector<size_t> input_strides(output_shape.size(), 1);
    std::vector<size_t> output_strides(output_shape.size(), 1);
    for (size_t axis = output_shape.size(); axis-- > 1;) {
        input_strides[axis - 1] = input_strides[axis] * padded_input_shape[axis];
        output_strides[axis - 1] = output_strides[axis] * output_shape[axis];
    }
    ...
    const char* input_ptr = static_cast<const char*>(input_buffer.get_data());
    char* output_ptr = static_cast<char*>(node.output_buffer.get_data());
    for (size_t output_index = 0; output_index < node.output_buffer.total_size; ++output_index) {
        size_t remainder = output_index;
        size_t input_index = 0;
        for (size_t axis = 0; axis < output_shape.size(); ++axis) {
            const size_t coord = remainder / output_strides[axis];
            remainder %= output_strides[axis];
            const size_t input_coord = padded_input_shape[axis] == 1 ? 0 : coord;
            input_index += input_coord * input_strides[axis];
        }
        std::memcpy(
            output_ptr + (output_index * element_bytes),
            input_ptr + (input_index * element_bytes),
            element_bytes
        );
    }
}

void compute_cumsum_node(
    GraphNode& node,
    const std::vector<std::unique_ptr<GraphNode>>& nodes,
    const std::unordered_map<size_t, size_t>& node_index_map
) {
    ...
    if (input_buffer.precision != Precision::FP16 && input_buffer.precision != Precision::FP32) {
        throw std::runtime_error("cumsum only supports FP16 and FP32 precision");
    }
