...
namespace {
    ...
    void matmul_fp16_inputs_to_fp32(
        const __fp16* lhs,
        const __fp16* rhs_transposed,
        float* output,
        size_t M,
        size_t K,
        size_t N
        ...
        CactusThreading::parallel_for(
            ...
            [&](size_t start_row, size_t end_row) {
                for (size_t m = start_row; m < end_row; ++m) {
                    const __fp16* lhs_row = lhs + (m * K);
                    float* out_row = output + (m * N);
                    for (size_t n = 0; n < N; ++n) {
                        const __fp16* rhs_row = rhs_transposed + (n * K);
                        float acc = 0.0f;
                        for (size_t k = 0; k < K; ++k) {
                            acc += static_cast<float>(lhs_row[k]) * static_cast<float>(rhs_row[k]);
                        }
                        out_row[n] = acc;
                    }
                }
    ...
    void matmul_fp16_inputs_to_fp16(
        const __fp16* lhs,
        const __fp16* rhs_transposed,
        __fp16* output,
        size_t M,
            ...
            [&](size_t start_row, size_t end_row) {
                for (size_t m = start_row; m < end_row; ++m) {
                    ...
                    for (size_t n = 0; n < N; ++n) {
                        ...
                        out_row[n] = static_cast<__fp16>(acc);
                    }
                }
            }
        );
    ...
    void matmul_fp32_inputs_to_fp32(
        const float* lhs,
        ...
        CactusThreading::parallel_for(
            ...
            [&](size_t start_row, size_t end_row) {
                for (size_t m = start_row; m < end_row; ++m) {
                    const float* lhs_row = lhs + (m * K);
                    float* out_row = output + (m * N);
                    for (size_t n = 0; n < N; ++n) {
                        const float* rhs_row = rhs_transposed + (n * K);
                        float acc = 0.0f;
                        for (size_t k = 0; k < K; ++k) {
    ...
    void matmul_fp32_inputs_to_fp16(
        const float* lhs,
        const float* rhs_transposed,
        __fp16* output,
        size_t M,
    ...
    const float* as_fp32_ptr(const BufferDesc& buffer, std::vector<float>& scratch) {
        if (buffer.precision == Precision::FP32) {
            return buffer.data_as<float>();
        }
        if (buffer.precision == Precision::FP16) {
            scratch.resize(buffer.total_size);
            cactus_fp16_to_fp32(buffer.data_as<__fp16>(), scratch.data(), buffer.total_size);
            return scratch.data();
        }
        throw std::runtime_error("matmul unsupported precision (expected FP16/FP32)");
    }

    void transpose_2d_fp32(const float* source, float* destination, size_t num_rows, size_t num_cols) {
        CactusThreading::parallel_for(
            num_rows,
...
void compute_matmul_node(GraphNode& node, const std::vector<std::unique_ptr<GraphNode>>& nodes, const std::unordered_map<size_t, size_t>& node_index_map) {
    ...
    if (PrecisionTraits::is_integer(rhs_buffer.precision) && rhs_buffer.group_size > 0) {
        ...
        if (node.output_buffer.precision != Precision::FP16) {
            throw std::runtime_error("Quantized matmul currently only supports FP16 outputs");
        }
        ...
        const bool fp16_fast_path =
            lhs_buffer.precision == Precision::FP16 && rhs_buffer.precision == Precision::FP16;
        if (fp16_fast_path) {
            ...
            if (pretransposed_rhs) {
                if (node.output_buffer.precision == Precision::FP16) {
                    matmul_fp16_inputs_to_fp16(lhs, rhs, node.output_buffer.data_as<__fp16>(), M, K, N);
                } else if (node.output_buffer.precision == Precision::FP32) {
                    matmul_fp16_inputs_to_fp32(lhs, rhs, node.output_buffer.data_as<float>(), M, K, N);
                } else {
                    throw std::runtime_error("FP16 matmul only supports FP16/FP32 outputs");
                }
            } else {
                ...
                if (node.output_buffer.precision == Precision::FP16) {
                    matmul_fp16_inputs_to_fp16(lhs, transpose_buffer_fp16.data(), node.output_buffer.data_as<__fp16>(), M, K, N);
                } else if (node.output_buffer.precision == Precision::FP32) {
                    matmul_fp16_inputs_to_fp32(lhs, transpose_buffer_fp16.data(), node.output_buffer.data_as<float>(), M, K, N);
                } else {
                    throw std::runtime_error("FP16 matmul only supports FP16/FP32 outputs");
                }
            }
        } else {
            std::vector<float> lhs_fp32_scratch;
            std::vector<float> rhs_fp32_scratch;
            const float* lhs = as_fp32_ptr(lhs_buffer, lhs_fp32_scratch);
            const float* rhs = as_fp32_ptr(rhs_buffer, rhs_fp32_scratch);
            const float* rhs_transposed = rhs;

            if (!pretransposed_rhs) {
                size_t transpose_size = rhs_shape[0] * rhs_shape[1];
                ensure_transpose_buffer_fp32(transpose_size);
                transpose_2d_fp32(rhs, transpose_buffer_fp32.data(), rhs_shape[0], rhs_shape[1]);
                rhs_transposed = transpose_buffer_fp32.data();
            }

            if (node.output_buffer.precision == Precision::FP32) {
                matmul_fp32_inputs_to_fp32(lhs, rhs_transposed, node.output_buffer.data_as<float>(), M, K, N);
            } else if (node.output_buffer.precision == Precision::FP16) {
                matmul_fp32_inputs_to_fp16(lhs, rhs_transposed, node.output_buffer.data_as<__fp16>(), M, K, N);
            } else {
                throw std::runtime_error("FP32 matmul only supports FP16/FP32 outputs");
            }
        }
    }
}
...
void compute_attention_node(GraphNode& node, const std::vector<std::unique_ptr<GraphNode>>& nodes, const std::unordered_map<size_t, size_t>& node_index_map) {
    ...
    if (mask_buffer) {
        ...
        if (mask_buffer->shape.size() == 3) {
            ...
            if (mask_buffer->shape[1] == 1) {
                mask_per_head = false;
            } else if (mask_buffer->shape[1] == num_q_heads) {
                mask_per_head = true;
            } else {
                throw std::runtime_error("Attention mask [B, H, T, S] head dimension mismatch");
            }
        } else {
...
void compute_conv2d_node(GraphNode& node, const std::vector<std::unique_ptr<GraphNode>>& nodes,
                         const std::unordered_map<size_t, size_t>& node_index_map) {
    if (node.params.backend == ComputeBackend::NPU) {
        throw std::runtime_error("NPU conv2d operation not yet implemented");
    }
    ...
    if (X.shape.size() != 4) {
        throw std::runtime_error("conv2d expects input [N, C_in, H, W]");
    }
    if (W.shape.size() != 4) {
        throw std::runtime_error("conv2d weight must be [C_out, C_in/groups, K_h, K_w]");
    }
    if (X.precision != Precision::FP16) {
        throw std::runtime_error("conv2d only supports FP16 activations");
    }

    const size_t stride = node.params.stride;
    const size_t padding = node.params.padding;
    const size_t dilation = node.params.dilation;
    const size_t groups = node.params.num_groups;
    if (stride == 0) {
        throw std::runtime_error("conv2d stride must be > 0");
    }
    if (dilation == 0) {
        throw std::runtime_error("conv2d dilation must be > 0");
    }
    if (groups == 0) {
        throw std::runtime_error("conv2d groups must be > 0");
    }

    const size_t N = X.shape[0];
    const size_t C_in = X.shape[1];
    const size_t H_in = X.shape[2];
    const size_t W_in = X.shape[3];
    const size_t C_out = W.shape[0];
    const size_t C_in_per_group = W.shape[1];
    const size_t K_h = W.shape[2];
    const size_t K_w = W.shape[3];

    if ((C_in % groups) != 0) {
        throw std::runtime_error("conv2d input channels must be divisible by groups");
    }
    if ((C_out % groups) != 0) {
        throw std::runtime_error("conv2d output channels must be divisible by groups");
    }
    if (C_in_per_group * groups != C_in) {
        throw std::runtime_error("conv2d weight must match [C_out, C_in/groups, K_h, K_w]");
    }

    const size_t effective_kh = dilation * (K_h - 1) + 1;
    const size_t effective_kw = dilation * (K_w - 1) + 1;
    if (H_in + 2 * padding < effective_kh || W_in + 2 * padding < effective_kw) {
        throw std::runtime_error("conv2d input spatial dimensions are too small for the requested kernel");
    }

    const size_t H_out_expected = (H_in + 2 * padding - effective_kh) / stride + 1;
    const size_t W_out_expected = (W_in + 2 * padding - effective_kw) / stride + 1;
    if (Y.shape.size() != 4 || Y.shape[0] != N || Y.shape[1] != C_out ||
        Y.shape[2] != H_out_expected || Y.shape[3] != W_out_expected) {
        throw std::runtime_error("conv2d output buffer shape mismatch");
    }
    Y.precision = Precision::FP16;

    const __fp16* x_ptr = X.data_as<__fp16>();
    __fp16* y_ptr = Y.data_as<__fp16>();

    auto finite_check_enabled = []() -> bool {
        static const bool enabled = []() {
            const char* env = std::getenv("CACTUS_CHECK_FINITE");
            return env != nullptr && env[0] != '\0' && env[0] != '0';
        }();
        return enabled;
    };

    auto throw_nonfinite_fp16 = [&](const char* label, const __fp16* data, size_t count) {
        if (!finite_check_enabled() || data == nullptr) {
            return;
        }
        for (size_t i = 0; i < count; ++i) {
            ...
            if (!std::isfinite(value)) {
                throw std::runtime_error(
                    std::string("conv2d detected non-finite ") + label +
    ...
    if (B) {
        ...
        if (B->precision == Precision::FP16) {
            ...
            throw std::runtime_error("conv2d bias only supports FP16/FP32");
        }
    }

    const size_t kernel_elems_per_out = C_in_per_group * K_h * K_w;
    ...
    if (W.precision == Precision::FP16) {
        ...
        const int8_t* w_int8 = W.data_as<int8_t>();
        w_fp16.resize(C_out * kernel_elems_per_out);
        if (W.is_grouped_int8()) {
            const __fp16* scales = W.scales_as_fp16();
            const size_t group_size = W.group_size;
            if (group_size == 0 || (kernel_elems_per_out % group_size) != 0 || scales == nullptr) {
                throw std::runtime_error("Grouped INT8 conv2d requires valid per-group scales");
            }
            const size_t num_qgroups = kernel_elems_per_out / group_size;
            for (size_t oc = 0; oc < C_out; ++oc) {
                for (size_t col = 0; col < kernel_elems_per_out; ++col) {
                    const size_t idx = oc * kernel_elems_per_out + col;
                    const size_t qgroup = col / group_size;
                    const float scale = static_cast<float>(scales[oc * num_qgroups + qgroup]);
                    w_fp16[idx] = static_cast<__fp16>(w_int8[idx] * scale);
                }
            }
        } else {
        ...
        throw std::runtime_error("conv2d only supports FP16/INT8 weights");
    }

    const size_t C_out_per_group = C_out / groups;
    const size_t C_in_group = C_in / groups;
    const size_t H_out = Y.shape[2];
    const size_t W_out = Y.shape[3];
    const size_t total_work = N * C_out;

    throw_nonfinite_fp16("input activation", x_ptr, X.total_size);
    throw_nonfinite_fp16("weight", w_ptr, C_out * kernel_elems_per_out);
    if (bias_ptr != nullptr) {
        throw_nonfinite_fp16("bias", bias_ptr, C_out);
    }

    CactusThreading::parallel_for(
        total_work,
        CactusThreading::Thresholds::SCALAR_EXPENSIVE,
        [&](size_t work_start, size_t work_end) {
            for (size_t work_idx = work_start; work_idx < work_end; ++work_idx) {
                const size_t n = work_idx / C_out;
                const size_t oc = work_idx % C_out;
                const size_t group = oc / C_out_per_group;
                const size_t ic_base = group * C_in_group;
                const size_t w_oc_base = oc * kernel_elems_per_out;

                for (size_t oh = 0; oh < H_out; ++oh) {
                    for (size_t ow = 0; ow < W_out; ++ow) {
                        float acc = bias_ptr ? static_cast<float>(bias_ptr[oc]) : 0.0f;
                        const size_t ih_origin = oh * stride;
                        const size_t iw_origin = ow * stride;

                        for (size_t ic_local = 0; ic_local < C_in_group; ++ic_local) {
                            const size_t ic = ic_base + ic_local;
                            const size_t w_ic_base = w_oc_base + ic_local * K_h * K_w;

                            for (size_t kh = 0; kh < K_h; ++kh) {
                                const long long ih = static_cast<long long>(ih_origin) +
                                    static_cast<long long>(kh * dilation) -
                                ...
                                for (size_t kw = 0; kw < K_w; ++kw) {
                                    const long long iw = static_cast<long long>(iw_origin) +
                                        static_cast<long long>(kw * dilation) -
                                        static_cast<long long>(padding);
                                    if (iw < 0 || iw >= static_cast<long long>(W_in)) {
                                        continue;
                                    }

                                    const size_t x_idx = ((n * C_in + ic) * H_in + static_cast<size_t>(ih)) * W_in + static_cast<size_t>(iw);
                                    const size_t w_idx = w_ic_base + kh * K_w + kw;
                                    acc += static_cast<float>(x_ptr[x_idx]) * static_cast<float>(w_ptr[w_idx]);
                                }
                            }
                        }

                        const size_t y_idx = ((n * C_out + oc) * H_out + oh) * W_out + ow;
                        y_ptr[y_idx] = static_cast<__fp16>(acc);
                    }
                }
            }
        }
    ...
    throw_nonfinite_fp16("output activation", y_ptr, Y.total_size);
}
