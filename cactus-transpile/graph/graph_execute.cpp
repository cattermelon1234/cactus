...
DECLARE_COMPUTE(compute_cumsum_node);
...
DECLARE_COMPUTE(compute_conv2d_node);
...
static const std::unordered_map<OpType, ComputeFn> dispatch_table = {
    ...
    {OpType::SCALAR_FLOOR_DIVIDE, compute_unary_op_node},
    ...
    {OpType::CUMSUM, compute_cumsum_node},
    ...
    {OpType::MASKED_SELECT_PREFIX, compute_masked_select_prefix_node},
    {OpType::MASKED_SCATTER, compute_masked_scatter_node},
    {OpType::EXPAND, compute_expand_node},
    {OpType::CONV2D, compute_conv2d_node},
...
static const char* get_op_name(OpType op) {
    switch (op) {
        ...
        case OpType::SCALAR_FLOOR_DIVIDE: return "SCALAR_FLOOR_DIVIDE";
        case OpType::MASKED_SELECT_PREFIX: return "MASKED_SELECT_PREFIX";
        case OpType::MASKED_SCATTER: return "MASKED_SCATTER";
...
static std::string format_node_debug(const GraphNode& node, const nodes_vector& nodes, const node_index_map_t& node_index_map) {
    std::ostringstream oss;
    oss << "node_id=" << node.id
        << " op=" << get_op_name(node.op_type)
        << " output_shape=" << format_shape(node.output_buffer.shape)
        << " output_elems=" << node.output_buffer.total_size;
    if (!node.input_ids.empty()) {
        ...
        for (size_t i = 0; i < node.input_ids.size(); ++i) {
            ...
            const size_t input_id = node.input_ids[i];
            oss << input_id;
            auto it = node_index_map.find(input_id);
            if (it != node_index_map.end()) {
                const auto& input_node = nodes[it->second];
                oss << ":" << get_op_name(input_node->op_type)
                    << ":" << format_shape(input_node->output_buffer.shape)
                    << ":" << input_node->output_buffer.total_size;
            }
        }
        oss << "]";
    }
...
void CactusGraph::execute(const std::string& profile_file) {
    ...
    bool trace_nodes = get_env_int("CACTUS_TRACE_NODES", 0) != 0;
    ...
    for (size_t node_idx = 0; node_idx < nodes_.size(); ++node_idx) {
        ...
        if (trace_nodes) {
            std::cerr << "[graph:node_begin] idx=" << node_idx
                      << " " << format_node_debug(*node, nodes_, node_index_map_)
                      << std::endl;
        }
        ...
        if (enable_profiling && node->op_type != OpType::INPUT) {
            ...
            try {
                ...
                throw std::runtime_error(
                    std::string(e.what()) + " | " + format_node_debug(*node, nodes_, node_index_map_)
                );
            }
        ...
        if (trace_nodes) {
            std::cerr << "[graph:node_end] idx=" << node_idx
                      << " node_id=" << node->id
                      << " op=" << get_op_name(node->op_type)
                      << std::endl;
        }
    ...
    if (trace_nodes) {
        std::cerr << "[graph:after_node_loop]" << std::endl;
    }
    ...
    if (trace_nodes) {
        std::cerr << "[graph:after_capture_decl]" << std::endl;
    }
    ...
    if (trace_nodes) {
        std::cerr << "[graph:execute_end]" << std::endl;
    }
}
